...
class TestQueryChainOrchestrator:
    ...
    async def test_execute_chain_semantic_search(self):
        ...
        mock_registry.invoke_tool = AsyncMock(
            return_value=[{"title": "Naruto", "anime_id": "naruto123", "score": 0.95}]
        )

        orchestrator = QueryChainOrchestrator(mock_registry)

        chain = QueryChain(
            chain_id="test_chain", queries=["action anime", "shounen series"]
        )
    ...
    async def test_execute_chain_similarity_search(self):
        ...
        mock_registry.invoke_tool = AsyncMock(
            side_effect=[
                ...
                [
                    {"title": "One Piece", "anime_id": "onepiece456", "score": 0.90}
                ],  # Similarity search
            ]
        )
    ...
    def test_calculate_query_confidence(self):
        ...
        results = [{"score": 0.9}, {"score": 0.8}, {"score": 0.85}]
...
class TestResultRefinementEngine:
    ...
    async def test_refine_results_basic(self):
        ...
        criteria = RefinementCriteria(
            min_confidence=0.7, max_iterations=1, target_result_count=5
        )

        context = AnimeSearchContext(query="action anime")

        refined_results, steps = await engine.refine_results(
            initial_results, criteria, context
        )
    ...
    async def test_refine_results_with_expansion(self):
        ...
        criteria = RefinementCriteria(
            min_confidence=0.7, target_result_count=5  # Need more results
        )
        ...
        mock_registry.invoke_tool.assert_called_with(
            "find_similar_anime",
            {
                "anime_id": "anime1",
                "limit": 2,
            },  # After multiple iterations, fewer needed
        )
...
class TestConversationFlowManager:
    ...
    async def test_execute_initial_search_stage(self):
        ...
        mock_registry = Mock(spec=MCPAdapterRegistry)
        mock_registry.invoke_tool = AsyncMock(
            return_value=[{"title": "Test Anime", "score": 0.9}]
        )
        ...
        mock_registry.invoke_tool.assert_called_once_with(
            "search_anime", {"query": "action anime", "limit": 15}
        )
    ...
    async def test_execute_multimodal_analysis_stage(self):
        ...
        mock_registry = Mock(spec=MCPAdapterRegistry)
        mock_registry.invoke_tool = AsyncMock(
            return_value=[{"title": "Multimodal Result", "score": 0.85}]
        )
        ...
        state.current_context = AnimeSearchContext(
            query="mecha anime", image_data="base64_image_data", text_weight=0.7
        )
    ...
    def test_extract_preferences_from_context(self):
        ...
        from src.langgraph.models import MessageType, WorkflowMessage

        state.messages = [
            WorkflowMessage(
                message_type=MessageType.USER, content="I love action anime"
            ),
            WorkflowMessage(
                message_type=MessageType.USER, content="Romance is also nice"
            ),
        ]
...
class TestSmartOrchestrationEngine:
    ...
    async def test_process_simple_conversation(self):
        ...
        mock_registry = Mock(spec=MCPAdapterRegistry)
        mock_registry.invoke_tool = AsyncMock(
            return_value=[{"title": "Simple Result", "score": 0.8}]
        )
        ...
        result_state = await engine.process_complex_conversation(
            state, "find action anime"
        )
    ...
    async def test_process_complex_conversation(self):
        ...
        mock_registry = Mock(spec=MCPAdapterRegistry)
        mock_registry.invoke_tool = AsyncMock(
            return_value=[{"title": "Complex Result", "score": 0.9}]
        )
    ...
    async def test_process_multimodal_orchestration(self):
        ...
        mock_registry = Mock(spec=MCPAdapterRegistry)
        mock_registry.invoke_tool = AsyncMock(
            return_value=[{"title": "Multimodal Result", "score": 0.88}]
        )
        ...
        result_state = await engine.process_multimodal_orchestration(
            state, "find mecha anime", "base64_image_data", 0.7
        )
    ...
    def test_assess_query_complexity(self):
        ...
        complex_query = (
            "find action anime and compare them but not horror first then show similar"
        )
...
class TestSmartOrchestrationModels:
    ...
    def test_query_chain_creation(self):
        ...
        chain = QueryChain(chain_id="test_chain", queries=["query1", "query2"])
    ...
    def test_smart_orchestration_state_extensions(self):
        ...
        success = state.add_to_chain(
            "chain_1", "follow-up query", {"type": "follow_up"}
        )
