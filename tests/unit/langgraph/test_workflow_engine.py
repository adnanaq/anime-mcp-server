...
class TestConversationalAgent:
    ...
    async def test_reasoning_node_with_none_years(self):
        ...
        state.current_context = AnimeSearchContext(
            query="test anime",
            results=[
                {"title": "Anime 1", "tags": ["action"], "studios": ["Studio A"], "year": 2020},
                {"title": "Anime 2", "tags": ["romance"], "studios": ["Studio B"], "year": None},
                {"title": "Anime 3", "tags": ["comedy"], "studios": ["Studio C"], "year": 2022},
...
class TestAnimeWorkflowEngine:
    ...
    async def test_conversation_context_preservation(self):
        ...
        mock_registry = Mock(spec=MCPAdapterRegistry)
        mock_registry.invoke_tool = AsyncMock(return_value=[
            {"title": "Contextual Result", "anime_id": "ctx123"}
        ])
        ...
        state2 = await engine.process_conversation(state1, "Find similar to the first result")
    ...
    async def test_preference_learning(self):
        ...
        mock_registry = Mock(spec=MCPAdapterRegistry)
        mock_registry.invoke_tool = AsyncMock(return_value=[
            {"title": "Action Anime", "tags": ["action", "shounen"]}
        ])
        ...
        state2 = await engine.process_conversation(state1, "Shounen is my favorite genre")
        
        # Should have learned preferences
        if state2.user_preferences:
            # Preferences should be inferred from conversation
            assert len(state2.user_preferences.favorite_genres) > 0 or len(state2.messages) >= 2