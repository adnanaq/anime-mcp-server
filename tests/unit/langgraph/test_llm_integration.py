...
from src.langgraph.models import ConversationState, AnimeSearchContext, WorkflowMessage, MessageType
...
class TestLLMIntegration:
    ...
    def conversation_state(self):
        ...
        state.add_message(WorkflowMessage(
            message_type=MessageType.USER,
            content="find 5 mecha anime from 2020s"
        ))
        return state
    ...
    async def test_extract_search_context_with_llm(self, agent):
        ...
        with patch('src.services.llm_service.extract_search_intent', 
                  new_callable=AsyncMock, return_value=mock_intent) as mock_extract:
            
            context = await agent._extract_search_context("find 5 mecha anime from 2020s")
    ...
    async def test_extract_search_context_multiple_types(self, agent):
        ...
        mock_intent = SearchIntent(
            query="movies and OVAs",
            anime_types=["Movie", "OVA"],
            confidence=0.85
        )
    ...
    async def test_extract_search_context_with_exclusions(self, agent):
        ...
        with patch('src.services.llm_service.extract_search_intent',
            ...
            context = await agent._extract_search_context("light comedy anime but not horror")
    ...
    @pytest.mark.asyncio
    async def test_understand_node_with_llm_integration(self, agent, conversation_state):
        ...
        with patch('src.services.llm_service.extract_search_intent',
            ...
            assert result_state.current_context.filters["genres"] == ["action", "adventure"]
    ...
    async def test_understand_node_preserves_existing_context(self, agent):
        ...
        with patch('src.services.llm_service.extract_search_intent',
            ...
            assert result_state.current_context.search_history == [{"previous": "search"}]
    ...
    async def test_realistic_query_processing(self):
        ...
        with patch('src.services.llm_service.extract_search_intent',
            ...
            result_state = await agent.process_user_message(
                state, 
                "find 5 mecha anime from late 90s to early 2000s"
            )
