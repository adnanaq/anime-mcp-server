...
from src.langgraph.models import ConversationState, MessageType, WorkflowMessage
...
class TestWorkflowEndpoints:
    ...
    def test_start_conversation(self, mock_workflow_engine):
        ...
        with patch(
            ...
            response = client.post(
                "/api/workflow/conversation", json={"message": "Find action anime"}
            )
    ...
    def test_continue_conversation(self, mock_workflow_engine):
        ...
        mock_state.messages = [
            WorkflowMessage(
                message_type=MessageType.USER, content="Find similar anime"
            ),
            WorkflowMessage(
                message_type=MessageType.ASSISTANT, content="Here are similar anime"
            ),
        ]
        mock_workflow_engine.process_conversation.return_value = mock_state
        ...
        with (
            patch(
                "src.api.workflow.get_workflow_engine",
    ...
    def test_multimodal_conversation(self, mock_workflow_engine):
        ...
        mock_state.messages = [
            WorkflowMessage(
                message_type=MessageType.USER, content="Find anime like this image"
            ),
            WorkflowMessage(
                message_type=MessageType.ASSISTANT,
                content="Found visually similar anime",
            ),
        ]
    ...
    def test_get_conversation_history(self, mock_workflow_engine):
        ...
        mock_workflow_engine.get_conversation_summary.return_value = (
            "User searched for action anime"
        )
        ...
        with patch("src.api.workflow.get_conversation_state") as mock_get_state:
            ...
            mock_state.messages = [
                WorkflowMessage(
                    message_type=MessageType.USER, content="Find action anime"
                )
            ]
    ...
    def test_invalid_message_format(self):
        ...
        client = TestClient(app)
        response = client.post("/api/workflow/conversation", json={"invalid": "format"})

        assert response.status_code == 422  # Validation error

    def test_workflow_engine_error(self, mock_workflow_engine):
        """Test handling workflow engine errors."""
        mock_workflow_engine.process_conversation.side_effect = Exception(
            "Engine error"
        )
        ...
        with patch(
            ...
            client = TestClient(app)
            response = client.post(
                "/api/workflow/conversation", json={"message": "Find anime"}
            )
...
class TestWorkflowRequestModels:
    ...
    def test_multimodal_request_validation(self):
        ...
        request = MultimodalRequest(
            message="Find mecha anime", image_data="base64_data", text_weight=0.8
        )
        ...
        # Default text weight
        request = MultimodalRequest(message="Find anime", image_data="base64_data")
...
class TestWorkflowIntegration:
    ...
    def test_workflow_uses_existing_mcp_tools(self, mock_workflow_engine):
        ...
        mock_state.messages = [
            WorkflowMessage(
                message_type=MessageType.ASSISTANT, content="Integration test response"
            )
        ]
