...
class TestAniSearchService:
    ...
    def anisearch_service(self):
        ...
        with (
            patch(
                "src.services.external.anisearch_service.AniSearchScraper"
    ...
    async def test_get_anime_details_success(
        self, anisearch_service, sample_anime_data
    ):
    ...
    async def test_get_anime_characters_success(
        self, anisearch_service, sample_character_data
    ):
        """Test successful anime characters retrieval."""
        # Setup
        anime_id = "cowboy-bebop"

        anisearch_service.scraper.get_anime_characters.return_value = (
    ...
    async def test_get_anime_recommendations_success(
        self, anisearch_service, sample_recommendations_data
    ):
        ...
        anisearch_service.scraper.get_anime_recommendations.return_value = (
            sample_recommendations_data
        )
        ...
        anisearch_service.scraper.get_anime_recommendations.assert_called_once_with(
            anime_id, limit
        )
    ...
    async def test_get_top_anime_success(
        self, anisearch_service, sample_top_anime_data
    ):
    ...
    async def test_get_top_anime_default_params(
        ...
        # Verify
        assert result == sample_top_anime_data
        anisearch_service.scraper.get_top_anime.assert_called_once_with(
            "highest_rated", 25
        )
    ...
    async def test_get_seasonal_anime_success(
        self, anisearch_service, sample_seasonal_data
    ):
        ...
        anisearch_service.scraper.get_seasonal_anime.assert_called_once_with(
            year, season, limit
        )

    @pytest.mark.asyncio
    async def test_get_seasonal_anime_default_params(
        self, anisearch_service, sample_seasonal_data
    ):
