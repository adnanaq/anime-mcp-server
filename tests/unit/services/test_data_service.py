...
class TestAnimeDataServiceCore:
    ...
    def test_extract_single_platform_id(
        self, service: AnimeDataService, platform: str, url: str, expected_id
    ):
    ...
    def test_extract_all_platform_ids_comprehensive(
        self,
        service: AnimeDataService,
        complex_anime_data: Dict[str, Any],
        expected_platform_ids: Dict[str, Any],
    ):
    ...
    def test_extract_platform_ids_mixed_valid_invalid(self, service: AnimeDataService):
        ...
        expected = {"myanimelist_id": 12345, "anilist_id": 67890}
        assert platform_ids == expected

    def test_process_anime_entry_with_platform_ids(
        self, service: AnimeDataService, complex_anime_data: Dict[str, Any]
    ):
    ...
    def test_create_embedding_text(
        self, service: AnimeDataService, anime_entry: AnimeEntry
    ):
    ...
    async def test_download_anime_database_success(
        self, service: AnimeDataService, mock_aiohttp_session
    ):
        """Test successful database download."""
        test_data = {"data": [{"title": "Test"}]}
        mock_aiohttp_session.get.return_value.__aenter__.return_value.text.return_value = (
            '{"data": [{"title": "Test"}]}'
        )
    ...
    @pytest.mark.asyncio
    async def test_download_anime_database_http_error(
        self, service: AnimeDataService, mock_aiohttp_session
    ):
...
class TestAnimeDataServiceEdgeCases:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "https://myanimelist.com/anime/123",
                "myanimelist",
            ),  # .com instead of .net
    ...
    def test_malformed_url_patterns(
        self, service: AnimeDataService, malformed_url: str, platform: str
    ):
    ...
    @pytest.mark.parametrize(
        "valid_variation,platform,expected_id",
        [
    ...
    def test_url_variation_handling(
        self,
        service: AnimeDataService,
        valid_variation: str,
        platform: str,
        ...
        if expected_id is None:
            assert (
                expected_key not in platform_ids or platform_ids[expected_key] is None
            )
        else:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "https://anime-planet.com/anime/serial-experiments-lain",
                "animeplanet",
                "serial-experiments-lain",
            ),
            (
                "https://anime-planet.com/anime/neon-genesis-evangelion",
                "animeplanet",
                "neon-genesis-evangelion",
            ),
            (
                "https://anime-planet.com/anime/a",
                "animeplanet",
                "a",
            ),  # Single character
            (
                "https://anime-planet.com/anime/123",
                "animeplanet",
                "123",
            ),  # Numeric slug
            (
                "https://anime-planet.com/anime/test_anime",
                "animeplanet",
                "test_anime",
            ),  # Underscore
            (
            ...
            (
                "https://notify.moe/anime/VERY-LONG-ID-STRING",
                "notify",
                "VERY-LONG-ID-STRING",
            ),
    ...
    def test_string_id_extraction(
        self, service: AnimeDataService, url: str, platform: str, expected_id: str
    ):
    ...
    def test_boundary_value_ids(
        self, service: AnimeDataService, id_value, platform: str
    ):
    ...
    def test_process_anime_entry_with_duplicate_sources(
        self, service: AnimeDataService
    ):
    ...
    def test_process_anime_entry_conflicting_platform_ids(
        self, service: AnimeDataService
    ):
    ...
    def test_create_embedding_text_with_special_characters(
        self, service: AnimeDataService
    ):
...
class TestAnimeDataServicePerformance:
    ...
    async def test_bulk_processing_performance(
        self, service: AnimeDataService, large_anime_dataset
    ):
    ...
    def mock_data_service(self):
        ...
        class MockDataService:
            ...
            async def _process_batch(
                self, batch: List[Dict[str, Any]], batch_num: int, config
            ):
            ...
            async def _process_batches_concurrently(
                self, batches: List[List[Dict[str, Any]]], config
            ):
            ...
            "data": [
                {
                    ...
                    "synopsis": "Humanity fights giant humanoids",
                },
                {
                    "title": "Death Note",
                    "synopsis": "A supernatural notebook that kills",
                },
                {"title": "One Piece", "synopsis": "Pirates searching for treasure"},
                {
                    "title": "Naruto",
                    "synopsis": "Young ninja's journey to become Hokage",
                },
                {"title": "Dragon Ball Z", "synopsis": "Saiyans protecting Earth"},
                {
                    "title": "invalid_anime",
                    "synopsis": "This will cause processing error",
                },
...
class TestRemoveEntriesFunctionality:
    ...
    async def test_remove_entries_success(
        self, mock_update_service, sample_remove_entries
    ):
    ...
    async def test_remove_entries_missing_anime_ids(self, mock_update_service):
        ...
        entries_without_ids = [
            {"title": "Attack on Titan", "synopsis": "No anime_id field"},
            {"anime_id": "", "title": "Death Note"},  # Empty anime_id
            {
                "title": "One Piece"
    ...
    async def test_remove_entries_batch_processing(self, mock_update_service):
        ...
        large_entry_list = [
            {"anime_id": f"anime{i:03d}", "title": f"Anime {i}"} for i in range(250)
        ]
