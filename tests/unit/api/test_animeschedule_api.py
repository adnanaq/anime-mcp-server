...
class TestAnimeScheduleAPI:
    ...
    def test_get_today_timetable_success(
        self, mock_service, client, sample_timetable_data
    ):
        ...
        # Execute
        response = client.get(
            "/external/animeschedule/today?timezone=Asia/Tokyo&region=JP"
        )
    ...
    @patch("src.api.external.animeschedule._animeschedule_service")
    def test_get_today_timetable_default_params(
        self, mock_service, client, sample_timetable_data
    ):
    ...
    def test_get_today_timetable_service_error(self, mock_service, client):
        ...
        # Setup
        mock_service.get_today_timetable = AsyncMock(
        ...
        assert (
            "AnimeSchedule today timetable service unavailable"
            in response.json()["detail"]
        )

    @patch("src.api.external.animeschedule._animeschedule_service")
    def test_get_timetable_by_date_success(
        self, mock_service, client, sample_timetable_data
    ):
        ...
        mock_service.get_timetable_by_date = AsyncMock(
            return_value=sample_timetable_data
        )
    ...
    def test_get_timetable_by_date_invalid_format(self, mock_service, client):
        ...
        # Setup
        invalid_date = (
            "2024-01-15"  # Valid format but will trigger ValueError from service
        )
        mock_service.get_timetable_by_date = AsyncMock(
    ...
    def test_get_seasonal_anime_invalid_season(self, mock_service, client):
        ...
        # Execute
        response = client.get(
            f"/external/animeschedule/seasonal/{invalid_season}/{year}"
        )
    ...
    @patch("src.api.external.animeschedule._animeschedule_service")
    def test_get_anime_schedule_success(
        self, mock_service, client, sample_schedule_data
    ):
    ...
    def test_get_anime_schedule_not_found(self, mock_service, client):
        ...
        assert (
            f"Anime schedule for ID {anime_id} not found" in response.json()["detail"]
        )
    ...
    def test_get_anime_schedule_service_error(self, mock_service, client):
        ...
        # Setup
        mock_service.get_anime_schedule = AsyncMock(
            side_effect=Exception("Service error")
        )
    ...
    @patch("src.api.external.animeschedule._animeschedule_service")
    def test_get_streaming_platforms_success(
        self, mock_service, client, sample_platforms_data
    ):
        """Test successful streaming platforms endpoint."""
        # Setup
        mock_service.get_streaming_platforms = AsyncMock(
            return_value=sample_platforms_data
        )
    ...
    def test_get_streaming_platforms_service_error(self, mock_service, client):
        ...
        assert response.status_code == 503
        assert (
            "AnimeSchedule platforms service unavailable" in response.json()["detail"]
        )
