...
class TestAnimePlanetAPI:
    ...
    def test_get_anime_recommendations_success(
        ...
        # Execute
        response = client.get(
            f"/external/animeplanet/anime/{anime_id}/recommendations?limit={limit}"
        )
    ...
    def test_get_top_anime_success(self, mock_service, client, sample_top_anime_data):
        ...
        # Execute
        response = client.get(
            f"/external/animeplanet/top?category={category}&limit={limit}"
        )
    ...
    def test_recommendations_validation_errors(self, client):
        ...
        # Test invalid limit (too high)
        response = client.get(
            "/external/animeplanet/anime/test/recommendations?limit=51"
        )
        assert response.status_code == 422

        # Test invalid limit (too low)
        response = client.get(
            "/external/animeplanet/anime/test/recommendations?limit=0"
        )
        assert response.status_code == 422
