...
class TestAniListClient:
        ...
        return {
            "data": {
                "Media": {
                    ...
                    "studios": {
                        "nodes": [
                            {"name": "Toei Animation", "isMain": True}
                        ]
                    },
                    "characters": {
                        "edges": [
    ...
    async def test_get_anime_by_id_success(self, anilist_client, sample_anime_response):
        ...
        with patch.object(anilist_client, '_make_graphql_request', new_callable=AsyncMock) as mock_request:
    ...
    @pytest.mark.asyncio
    async def test_search_anime_with_filters(self, anilist_client, sample_search_response):
    ...
    async def test_graphql_request_with_auth(self, mock_dependencies):
        ...
        with patch('aiohttp.ClientSession.post') as mock_post:
            ...
            result = await client._make_graphql_request(query=query, variables=variables)
    ...
    async def test_cache_integration(self, anilist_client, sample_anime_response):
        ...
        # Mock cache hit
        anilist_client.cache_manager.get = AsyncMock(return_value=sample_anime_response["data"]["Media"])
