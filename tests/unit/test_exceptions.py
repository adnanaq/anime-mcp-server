...
class TestCustomExceptions:
    ...
    def test_anime_server_error_base_functionality(self):
        ...
        error_with_details = MockAnimeServerError(
            "Detailed error", error_code="TEST_ERROR", details=details
        )
    ...
    def test_vector_database_error_hierarchy(self):
        ...
        class MockQdrantCollectionError(MockVectorDatabaseError):
            def __init__(
                self, collection_name: str, operation: str, message: str = None
            ):
                self.collection_name = collection_name
                self.operation = operation
                default_message = (
                    f"Collection '{collection_name}' {operation} operation failed"
                )
                super().__init__(
                    message or default_message,
        ...
        assert (
            "Failed to connect to Qdrant at http://localhost:6333" in conn_error.message
        )
        ...
        assert (
            "Collection 'anime_database' create operation failed" in coll_error.message
        )
    ...
    def test_embedding_generation_error(self):
        ...
        assert f"Failed to generate embedding for text using model {model}" in str(
            error
        )
    ...
    def test_data_processing_errors(self):
        ...
        class MockAnimeDataValidationError(MockDataProcessingError):
            def __init__(
                self, anime_id: str, field: str, value: Any, message: str = None
            ):
        ...
        download_error = MockAnimeDataDownloadError(
            "https://example.com/anime.json", status_code=404
        )
        assert (
            "Failed to download anime data from https://example.com/anime.json (HTTP 404)"
            in str(download_error)
        )
        ...
        validation_error = MockAnimeDataValidationError("anime123", "episodes", -1)
    ...
    def test_platform_id_extraction_error(self):
        ...
        error = MockPlatformIDExtractionError(
            "myanimelist", "https://myanimelist.net/anime/invalid"
        )

        assert (
            "Failed to extract myanimelist ID from URL: https://myanimelist.net/anime/invalid"
            in str(error)
        )
    ...
    def test_configuration_errors(self):
        ...
        class MockMissingConfigurationError(MockConfigurationError):
            def __init__(self, setting: str, message: str = None):
                self.setting = setting
                default_message = (
                    f"Required configuration setting '{setting}' is missing"
                )
                super().__init__(
                    message or default_message,
        ...
        assert (
            "Invalid configuration for qdrant_url: URL must start with http:// or https://"
            in str(invalid_error)
        )
        assert invalid_error.error_code == "INVALID_CONFIGURATION"
        assert invalid_error.details["setting"] == "qdrant_url"
        assert (
            invalid_error.details["reason"] == "URL must start with http:// or https://"
        )

        # Test missing configuration error
        missing_error = MockMissingConfigurationError("QDRANT_URL")
        assert "Required configuration setting 'QDRANT_URL' is missing" in str(
            missing_error
        )
    ...
    def test_mcp_errors(self):
        ...
        assert "MCP tool 'search_anime' failed: Vector database unavailable" in str(
            tool_error
        )
            ...
            if status_code == 400:
                if "query" in kwargs:
                    return MockHTTPException(
                        f"Invalid query: {kwargs['query']}", status_code, **kwargs
                    )
                return MockHTTPException("Bad Request", status_code, **kwargs)
            elif status_code == 404:
                if "identifier" in kwargs:
                    return MockHTTPException(
                        f"Not found: {kwargs['identifier']}", status_code, **kwargs
                    )
                ...
                if "url" in kwargs:
                    return MockHTTPException(
                        f"Service unavailable: {kwargs['url']}", status_code, **kwargs
                    )
    ...
    def test_exception_handler_decorator_logic(self):
        ...
        assert "Unexpected error in function_with_generic_error: Generic error" in str(
            exc_info.value
        )
