...
class TestPlatformCrossReference:
    ...
    @pytest.mark.e2e
    @pytest.mark.slow
    async def test_complete_data_pipeline_with_crossref(
        self, sample_anime_with_all_platforms
    ):
    ...
    def test_partial_platform_coverage(self, client: TestClient):
        ...
        with patch("src.main.qdrant_client") as mock_client:
            ...
            response = client.post(
                "/api/search/semantic", json={"query": "partial coverage", "limit": 10}
            )
    ...
    def test_crossref_data_quality_impact(self):
        ...
        assert (
            high_quality_processed["data_quality_score"]
            > low_quality_processed["data_quality_score"]
        )

        # High quality should have multiple platform IDs
        platform_id_count_high = sum(
            1
            for key in high_quality_processed.keys()
            if key.endswith("_id") and high_quality_processed[key] is not None
        )

        platform_id_count_low = sum(
            1
            for key in low_quality_processed.keys()
