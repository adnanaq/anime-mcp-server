...
class TestAniDBService:
    ...
    def anidb_service(self):
        ...
        with (
            patch(
                "src.services.external.anidb_service.AniDBClient"
    ...
    async def test_search_anime_success(self, anidb_service, sample_search_results):
        ...
        anidb_service.client.search_anime_by_name.return_value = sample_search_results[
            0
        ]
    ...
    async def test_get_anime_characters_success(
        self, anidb_service, sample_character_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_similar_anime_success(
        self, anidb_service, sample_search_results
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_similar_anime_default_limit(
        self, anidb_service, sample_search_results
    ):
    ...
    async def test_health_check_unhealthy(self, anidb_service):
        ...
        # Setup
        anidb_service.client.get_anime_by_id.side_effect = Exception(
            "Connection timeout"
        )
