...
class TestAnimeCountdownService:
    ...
    def animecountdown_service(self):
        ...
        with (
            patch(
                "src.services.external.animecountdown_service.AnimeCountdownScraper"
    ...
    async def test_search_anime_success(
        self, animecountdown_service, sample_search_results
    ):
        ...
        animecountdown_service.scraper.search_anime = AsyncMock(
    ...
    async def test_search_anime_failure(self, animecountdown_service):
        ...
        # Setup
        animecountdown_service.scraper.search_anime.side_effect = Exception(
            "Scraping error"
        )
    ...
    async def test_get_anime_details_success(
        self, animecountdown_service, sample_anime_data
    ):
        """Test successful anime details retrieval."""
        # Setup
        anime_id = "cowboy-bebop"

        animecountdown_service.scraper.get_anime_by_slug.return_value = (
            sample_anime_data
        )
        ...
        animecountdown_service.scraper.get_anime_by_slug.assert_called_once_with(
            anime_id
        )
    ...
    @pytest.mark.asyncio
    async def test_get_currently_airing_success(
        self, animecountdown_service, sample_airing_data
    ):
        """Test successful currently airing anime retrieval."""
        # Setup
        limit = 15

        animecountdown_service.scraper.get_currently_airing.return_value = (
            sample_airing_data
        )
        ...
        animecountdown_service.scraper.get_currently_airing.assert_called_once_with(
            limit
        )

    @pytest.mark.asyncio
    async def test_get_currently_airing_default_limit(
        self, animecountdown_service, sample_airing_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_upcoming_anime_success(
        self, animecountdown_service, sample_upcoming_data
    ):
        """Test successful upcoming anime retrieval."""
        # Setup
        limit = 10

        animecountdown_service.scraper.get_upcoming_anime.return_value = (
            sample_upcoming_data
        )
    ...
    @pytest.mark.asyncio
    async def test_get_upcoming_anime_default_limit(
        self, animecountdown_service, sample_upcoming_data
    ):
        ...
        # Verify
        assert result == sample_upcoming_data
        animecountdown_service.scraper.get_upcoming_anime.assert_called_once_with(
            20
        )  # Default limit
    ...
    @pytest.mark.asyncio
    async def test_get_popular_anime_success(
        self, animecountdown_service, sample_popular_data
    ):
        ...
        animecountdown_service.scraper.get_popular_anime.return_value = (
            sample_popular_data
        )
        ...
        animecountdown_service.scraper.get_popular_anime.assert_called_once_with(
            time_period, limit
        )

    @pytest.mark.asyncio
    async def test_get_popular_anime_default_params(
        self, animecountdown_service, sample_popular_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_anime_countdown_success(
        self, animecountdown_service, sample_countdown_data
    ):
        """Test successful anime countdown retrieval."""
        # Setup
        anime_id = "demon-slayer-s3"

        animecountdown_service.scraper.get_anime_countdown.return_value = (
            sample_countdown_data
        )
        ...
        animecountdown_service.scraper.get_anime_countdown.assert_called_once_with(
            anime_id
        )
    ...
    async def test_scraper_has_required_methods(self, animecountdown_service):
        ...
        for method_name in required_methods:
            assert hasattr(
                scraper, method_name
            ), f"Scraper missing required method: {method_name}"
        ...
        for method_name in specialized_methods:
            assert hasattr(
                scraper, method_name
            ), f"Scraper missing specialized method: {method_name}"
