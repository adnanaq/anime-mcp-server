...
class TestAnimeCountdownRealData:
    ...
    async def test_upcoming_anime_returns_data(self):
        ...
        assert len(upcoming) > 0, "No upcoming anime found - scraper needs implementation"
    ...
    async def test_search_returns_data(self):
        ...
        assert len(results) > 0, "No search results for 'demon slayer' - scraper needs implementation"
