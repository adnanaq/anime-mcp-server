...
from src.vector.vision_processor import VisionProcessor, MockVisionProcessor
...
class TestPhase4Integration:
    ...
    @pytest.mark.asyncio
    async def test_new_image_search_methods(self, mock_settings_multi_vector, sample_image_base64):
        ...
        with patch('src.vector.qdrant_client.QdrantSDK') as mock_sdk, \
            ...
            with patch.object(QdrantClient, '_ensure_collection_exists'):
                ...
                results = await qdrant_client.search_by_image(sample_image_base64, limit=5)
                assert isinstance(results, list)  # Should call search method
                
                # Test find visually similar anime
                results = await qdrant_client.find_visually_similar_anime("test_id", limit=5)
                assert isinstance(results, list)  # Should call find_similar as fallback when no image vector
                
                # Test multimodal search
                results = await qdrant_client.search_multimodal("test query", sample_image_base64, limit=5)
                assert isinstance(results, list)
    
    @pytest.mark.asyncio
    async def test_graceful_fallback_without_multi_vector(self, mock_settings_single_vector, sample_image_base64):
        ...
        with patch('src.vector.qdrant_client.QdrantSDK') as mock_sdk, \
            ...
            with patch.object(QdrantClient, '_ensure_collection_exists'):
                ...
                results = await qdrant_client.search_multimodal("test query", sample_image_base64)
                assert len(results) == 1
    ...
    @pytest.mark.asyncio
    async def test_collection_creation_modes(self, mock_settings_single_vector, mock_settings_multi_vector):
        ...
        with patch('src.vector.qdrant_client.QdrantSDK') as mock_sdk, \
            ...
            with patch.object(QdrantClient, '_init_vision_processor'):
                qdrant_client_single = QdrantClient(settings=mock_settings_single_vector)
                ...
                with patch('src.vector.vision_processor.VisionProcessor') as mock_vision:
                    mock_vision.return_value = MockVisionProcessor()
                    qdrant_client_multi = QdrantClient(settings=mock_settings_multi_vector)
    ...
    def test_zero_breaking_changes_guarantee(self):
        ...
        required_methods = [
            'health_check', 'get_stats', 'add_documents', 'search',
            'get_by_id', 'find_similar', 'clear_index', 'delete_collection',
            'create_collection', 'get_similar_anime'
        ]
