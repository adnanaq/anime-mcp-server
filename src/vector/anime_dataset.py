...
class AnimeDataset(Dataset):
    ...
    def _prepare_samples(self) -> List[DataSample]:
        ...
        for anime in self.anime_data:
            try:
                ...
                logger.warning(f"Error processing anime {anime.get('title', 'unknown')}: {e}")
                continue
    ...
    def _extract_character_labels(self, anime: Dict[str, Any]) -> List[str]:
        ...
        tags = anime.get('tags', [])
        for tag in tags:
            if any(keyword in tag.lower() for keyword in ['character', 'protagonist', 'main']):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'title': sample.title,
            'text': sample.text,
            'text_embedding': torch.tensor(text_embedding) if text_embedding else None,
            'image_embedding': torch.tensor(image_embedding) if image_embedding else None,
            'character_labels': character_labels,
            'art_style_label': art_style_label,
            'genre_labels': genre_labels,
            'metadata': {
