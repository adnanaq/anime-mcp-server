...
async def get_anime_details(
    anime_id: str = Path(
        ..., description="AnimeCountdown anime ID or slug", min_length=1
    )
):
...
async def get_currently_airing(
    limit: int = Query(
        25, description="Maximum number of results (1-100)", ge=1, le=100
    )
):
    ...
    try:
        ...
        return AnimeCountdownListResponse(
            results=results, total=len(results), limit=limit
        )
...
async def get_upcoming_anime(
    limit: int = Query(
        20, description="Maximum number of results (1-100)", ge=1, le=100
    )
):
...
async def get_popular_anime(
    time_period: TimePeriod = Query(
        TimePeriod.ALL_TIME, description="Time period for popularity ranking"
    ),
    limit: int = Query(
        25, description="Maximum number of results (1-100)", ge=1, le=100
    ),
    ...
    try:
        logger.info(
            "AnimeCountdown popular anime request: time_period='%s', limit=%d",
            time_period,
            limit,
        )
        results = await animecountdown_service.get_popular_anime(
            time_period.value, limit
        )
