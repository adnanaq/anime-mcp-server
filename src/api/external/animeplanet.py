...
router = APIRouter(
    prefix="/external/animeplanet", tags=["External APIs", "Anime-Planet"]
)
...
async def search_anime(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Anime-Planet search service unavailable: {str(e)}"
        )
...
async def get_anime_details(
    ...
    try:
        ...
        return {"source": "animeplanet", "anime_id": anime_id, "data": anime_data}
        ...
        raise HTTPException(
            status_code=503, detail=f"Anime-Planet service unavailable: {str(e)}"
        )
...
async def get_anime_recommendations(
    ...
    try:
        logger.info(
            "Anime-Planet recommendations: anime_id='%s', limit=%d", anime_id, limit
        )

        recommendations = await _animeplanet_service.get_anime_recommendations(
            anime_id, limit
        )
...
async def get_top_anime(
    category: str = Query(
        "top-anime",
        description="Category (top-anime, most-watched, highest-rated, etc.)",
    ),
