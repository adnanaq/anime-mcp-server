...
async def search_anime(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"AniSearch search service unavailable: {str(e)}"
        )
...
async def get_anime_details(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"AniSearch service unavailable: {str(e)}"
        )
...
async def get_anime_recommendations(
    ...
    """
    try:
        logger.info("AniSearch recommendations: anime_id='%s', limit=%d", anime_id, limit)
        
        recommendations = await _anisearch_service.get_anime_recommendations(anime_id, limit)
...
async def get_top_anime(
    category: str = Query("highest_rated", description="Category (highest_rated, most_popular, newest, etc.)"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"AniSearch top anime service unavailable: {str(e)}"
        )
...
async def get_seasonal_anime(
    ...
    """
    try:
        logger.info("AniSearch seasonal anime: year=%s, season='%s', limit=%d", year, season, limit)
        
        seasonal_anime = await _anisearch_service.get_seasonal_anime(year, season, limit)
        ...
        raise HTTPException(
            status_code=503,
            detail=f"AniSearch seasonal service unavailable: {str(e)}"
        )
