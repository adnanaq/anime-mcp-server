...
async def search_anime(
    ...
    try:
        ...
        results = await _kitsu_service.search_anime(query=q, limit=limit)
        ...
        raise HTTPException(
            status_code=503, detail=f"Kitsu search service unavailable: {str(e)}"
        )
...
async def get_anime_details(
    ...
    try:
        ...
        if not anime_data:
            raise HTTPException(
                status_code=404, detail=f"Anime with ID {anime_id} not found on Kitsu"
            )

        return {"source": "kitsu", "anime_id": anime_id, "data": anime_data}
        ...
        raise HTTPException(
            status_code=503, detail=f"Kitsu service unavailable: {str(e)}"
        )
