...
async def search_anime(
    ...
    """
    try:
        results = await _anilist_service.search_anime(query=q, limit=limit, page=page)
        ...
        raise HTTPException(
            status_code=503, detail=f"AniList search service unavailable: {str(e)}"
        )
...
async def get_anime_details(anime_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=404, detail=f"Anime with ID {anime_id} not found on AniList"
            )

        return {"source": "anilist", "anime_id": anime_id, "data": result}
        ...
        raise HTTPException(
            status_code=503, detail=f"AniList service unavailable: {str(e)}"
        )


@router.get("/trending")
async def get_trending_anime(
    limit: int = Query(20, ge=1, le=50), page: int = Query(1, ge=1)
) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"AniList trending service unavailable: {str(e)}"
        )
...
async def get_upcoming_anime(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"AniList upcoming service unavailable: {str(e)}"
        )
...
async def get_popular_anime(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"AniList popular service unavailable: {str(e)}"
        )
...
async def get_staff_details(staff_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=404, detail=f"Staff with ID {staff_id} not found on AniList"
            )

        return {"source": "anilist", "staff_id": staff_id, "data": result}
...
async def get_studio_details(studio_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"source": "anilist", "studio_id": studio_id, "data": result}
