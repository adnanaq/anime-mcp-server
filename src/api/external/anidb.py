...
async def search_anime(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"AniDB search service unavailable: {str(e)}"
        )
...
async def get_anime_details(
    ...
    try:
        ...
        if anime_data is None:
            raise HTTPException(
                status_code=404, detail=f"Anime with ID {anime_id} not found on AniDB"
            )

        return {"source": "anidb", "anime_id": anime_id, "data": anime_data}
        ...
        raise HTTPException(
            status_code=503, detail=f"AniDB service unavailable: {str(e)}"
        )
...
async def get_anime_characters(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"AniDB characters service unavailable: {str(e)}"
        )
...
async def get_anime_episodes(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"AniDB episodes service unavailable: {str(e)}"
        )
...
async def get_similar_anime(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"AniDB similar anime service unavailable: {str(e)}"
        )
...
async def get_random_anime() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"source": "anidb", "type": "random", "anime": random_anime}

    except Exception as e:
        logger.error("AniDB random anime failed: %s", e)
        raise HTTPException(
            status_code=503, detail=f"AniDB random anime service unavailable: {str(e)}"
        )
