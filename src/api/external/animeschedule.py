...
router = APIRouter(
    prefix="/external/animeschedule", tags=["External APIs", "AnimeSchedule"]
)
...
async def get_today_timetable(
    timezone: Optional[str] = Query(
        None, description="Timezone for schedule (e.g., 'Asia/Tokyo')"
    ),
    ...
    try:
        logger.info(
            "AnimeSchedule today timetable: timezone=%s, region=%s", timezone, region
        )
...
async def get_anime_schedule(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"AnimeSchedule service unavailable: {str(e)}"
        )
