...
class LangGraphErrorHandler:
    ...
    async def handle_parameter_extraction_failure(
        self, query: str, attempt: int
    ) -> Dict[str, Any]:
    ...
    async def detect_state_corruption(self, state: Dict[str, Any]) -> bool:
        ...
        if "results" in state and not isinstance(
            state["results"], (list, dict, type(None))
        ):
            return True

        return False

    async def handle_graph_timeout(
        self, execution_id: str, partial_results: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def prune_conversation_history(
        self, state: Dict[str, Any], keep_last_n: int = 10
    ) -> Dict[str, Any]:
    ...
    async def force_tool_change(
        self, current_tool: str, alternative_tools: List[str]
    ) -> str:
