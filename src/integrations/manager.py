...
class IntegrationsManager:
    ...
    def __init__(self, settings: Optional[Settings] = None):
        ...
        logger.info("IntegrationsManager initialized with %d API clients and %d scrapers",
                   len(self.api_clients), len(self.scrapers))
        ...
        for source in sources_to_search:
            if source in self.scrapers:
                ...
                if hasattr(scraper, 'search_anime'):
                    ...
                    task = self._search_countdowns_with_scraper(scraper, source, query, limit)
                    scraper_tasks.append(task)
    ...
    async def get_anime_details(
        ...
        if fallback_sources:
            for fallback_source in fallback_sources:
                ...
                if result:
                    logger.info("Found anime details using fallback source: %s", fallback_source)
                    return result
    ...
    async def _get_anime_from_source(
        ...
        try:
            ...
            if source in self.scrapers:
                ...
                elif hasattr(scraper, 'get_anime_countdown_by_slug') and isinstance(anime_id, str):
                    return await scraper.get_anime_countdown_by_slug(anime_id)
                elif hasattr(scraper, 'get_anime_countdown_by_id') and isinstance(anime_id, int):
        ...
        for name, client in self.api_clients.items():
            try:
                ...
                if hasattr(client, 'health_check'):
                    ...
                    health_status['api_clients'][name] = not self.circuit_breaker.is_open()
            except Exception:
