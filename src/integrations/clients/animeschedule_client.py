...
class AnimeScheduleClient(BaseClient):
    ...
    async def _make_request(self, endpoint: str, params: Optional[Dict[str, Any]] = None, timeout: int = 30) -> Dict[str, Any]:
            ...
            async with session.get(url, headers=headers, params=params) as response:
                ...
                if response.status != 200:
                    try:
                        ...
                        raise Exception(f"AnimeSchedule API HTTP {response.status} error")
                
                return await response.json()
    
    async def get_today_timetable(self, timezone: Optional[str] = None, region: Optional[str] = None) -> Dict[str, Any]:
