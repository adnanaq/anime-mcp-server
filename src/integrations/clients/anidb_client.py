...
class AniDBClient(BaseClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(service_name="anidb", **kwargs)
    ...
    async def authenticate(self) -> str:
        ...
        params = {"request": "auth", "user": self.username, "pass": self.password}
    ...
    async def get_anime_by_id(self, anime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            params = {"request": "anime", "aid": anime_id}
    ...
    def _parse_anime_xml(self, xml_content: str) -> Dict[str, Any]:
        ...
        anime_data = {
            ...
            "episodecount": (
                root.find("episodecount").text
                if root.find("episodecount") is not None
                else None
            ),
            "startdate": (
            ...
            "enddate": (
                root.find("enddate").text if root.find("enddate") is not None else None
            ),
            "description": (
    ...
    async def logout(self) -> bool:
        ...
        try:
            params = {"request": "logout", "s": self.session_key}
    ...
    def _parse_episode_xml(self, xml_content: str) -> Dict[str, Any]:
        ...
        episode_data = {
            ...
            "length": (
                root.find("length").text if root.find("length") is not None else None
            ),
            "airdate": (
                root.find("airdate").text if root.find("airdate") is not None else None
            ),
            "rating": (
                root.find("rating").text if root.find("rating") is not None else None
            ),
            "summary": (
                root.find("summary").text if root.find("summary") is not None else None
            ),
        }
    ...
    def _parse_characters_xml(self, xml_content: str) -> List[Dict[str, Any]]:
        ...
        for character in root.findall(".//character"):
            character_data = {
                ...
                "name": (
                    character.find("name").text
                    if character.find("name") is not None
                    else None
                ),
                "gender": (
                    character.find("gender").text
                    if character.find("gender") is not None
                    else None
                ),
                "description": (
                    character.find("description").text
                    if character.find("description") is not None
                    else None
                ),
                "picture": (
                    character.find("picture").text
                    if character.find("picture") is not None
                    else None
                ),
            }
