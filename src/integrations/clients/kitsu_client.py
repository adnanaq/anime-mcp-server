...
class KitsuClient(BaseClient):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, params=params) as response:
                if response.status == 429:
                    ...
                    raise Exception(f"Kitsu rate limit exceeded. Retry after {retry_after} seconds")
                
                if response.status == 404:
                    error_data = await response.json()
                    if "errors" in error_data:
                        error_msg = error_data["errors"][0].get("title", "Record not found")
                ...
                if response.status != 200:
                    try:
                        error_data = await response.json()
                        if "errors" in error_data:
                            error_msg = error_data["errors"][0].get("title", "Unknown error")
                            raise Exception(f"Kitsu API error: {error_msg}")
                    except:
    ...
    async def get_anime_by_id(self, anime_id: int, includes: Optional[List[str]] = None) -> Optional[Dict[str, Any]]:
        """Get anime by ID."""
        # Check cache first
        ...
        try:
            ...
            response = await self._make_request(endpoint, params=params if params else None)
    ...
    async def get_anime_characters(self, anime_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response and "data" in response and "included" in response:
                ...
                characters = []
                included_map = {item["id"]: item for item in response["included"] if item["type"] == "characters"}
                
                for char_rel in response["data"]:
                    char_data = char_rel.copy()
                    if "relationships" in char_rel and "character" in char_rel["relationships"]:
                        char_ref = char_rel["relationships"]["character"]["data"]
                        if char_ref["id"] in included_map:
                            char_data["character"] = included_map[char_ref["id"]]["attributes"]
