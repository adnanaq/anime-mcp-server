...
class AnimeCountdownScraper(BaseScraper):
    ...
    async def get_anime_countdown_by_id(self, anime_id: int) -> Optional[Dict[str, Any]]:
    ...
    async def search_anime_countdowns(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    async def get_currently_airing(self, limit: int = 25) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for anime in upcoming:
                ...
                if countdown_info.get('is_airing') or countdown_info.get('next_episode'):
    ...
    async def get_popular_anime(self, time_period: str = "all_time", limit: int = 25) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            popular_terms = ["demon slayer", "attack on titan", "naruto", "one piece", "dragon ball"]
            all_results = []
        ...
        for container in countdown_containers:
            ...
            if not countdown_data:
                ...
                if time_match:
                    ...
                    hours_match = re.search(r'(\d+)\s*hours?\s*(?:left|remaining)', text)
                    if hours_match:
    ...
    def _parse_single_search_result(self, container) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            countdown_elem = (
                container.find('div', class_='countdown-preview') or
                container.find('span', class_='countdown')
            )
            if countdown_elem:
