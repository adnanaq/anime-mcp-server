...
class AniSearchScraper(BaseScraper):
            ...
            try:
                ...
                if "startDate" in json_data and "endDate" in json_data:
                    ...
                    if not data.get("status"):
                        data["status"] = self._derive_status_from_dates(start_date, end_date)
    ...
    def _extract_title_variants(self, soup) -> Dict[str, Any]:
        ...
        for elem in title_elements:
            ...
            if (len(title_text.split()) == 1 and 
                len(title_text) < 10 and 
                title_text.isupper() and 
                'details' in parent_classes):
                continue
        ...
        if duration_info:
            ...
            if episodes and duration_info:
                try:
                    ...
                    duration_minutes = int(''.join(filter(str.isdigit, str(duration_info))))
                    episode_count = int(str(episodes))
                    total_minutes = duration_minutes * episode_count
                    data["total_runtime"] = f"{total_minutes} minutes ({total_minutes // 60}h {total_minutes % 60}m)"
                except (ValueError, TypeError):
                    pass
    ...
    def _extract_streaming_platforms(self, soup) -> List[Dict[str, str]]:
        ...
        streaming_keywords = ["crunchyroll", "netflix", "amazon", "funimation", "hulu", "stream"]
        ...
        for link in soup.find_all("a", href=True):
            ...
            if any(keyword in href.lower() or keyword in text.lower() for keyword in streaming_keywords):
                if href.startswith("http") and text:
