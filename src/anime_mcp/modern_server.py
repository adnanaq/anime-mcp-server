...
async def discover_anime(
    ...
    try:
        ...
        result = await anime_swarm.discover_anime(
            query=query, user_context=user_preferences, session_id=session_id
        )
        ...
        if ctx:
            ...
            await ctx.info(
                f"Discovery completed: {total_results} results using {len(agents_used)} agents"
            )
...
async def get_currently_airing_anime(
    ...
    try:
        ...
        if ctx:
            await ctx.info(
                f"Found {result.get('total_results', 0)} currently airing anime"
            )
...
async def find_similar_anime_workflow(
    ...
    if ctx:
        await ctx.info(
            f"Finding anime similar to: '{reference_anime}' ({similarity_mode} mode)"
        )
...
async def search_by_streaming_platform(
    ...
    try:
        ...
        if ctx:
            await ctx.info(
                f"Found {result.get('total_results', 0)} anime on specified platforms"
            )
...
async def platforms_status() -> str:
    ...
    status = {
        "mal": {
            "status": (
                "available" if hasattr(settings, "mal_api_key") else "requires_api_key"
            )
        },
        "anilist": {
            "status": (
                "available" if hasattr(settings, "anilist_token") else "requires_token"
            )
        },
        ...
        "langgraph_workflows": {
            "status": "available" if anime_swarm else "initializing"
        },
    }
...
async def workflow_architecture() -> str:
    ...
    architecture = {
        ...
        "tools_per_agent": {"SearchAgent": 20, "ScheduleAgent": 6, "Total": 26},
    }
...
async def initialize_server():
    ...
    try:
        ...
        logger.info(
            "✅ Multi-agent workflow system initialized with manual swarm creation"
        )

        # Log final capabilities
        total_tools = (
            4 + tiered_count + platform_count
        )  # 4 workflow + tiered + platform
        logger.info(f"🎯 Tool Architecture: 4-tier progressive complexity")
        logger.info(f"🤖 Agents: SearchAgent, ScheduleAgent")
        logger.info(
            f"🔧 Total tools: {total_tools} (4 workflow + {tiered_count} tiered + {platform_count} platform)"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--port", type=int, default=8001, help="Port for SSE mode (default: 8001)"
    )
    parser.add_argument(
