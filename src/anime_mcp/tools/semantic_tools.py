...
from ...models.anime import SearchResult
...
async def anime_semantic_search(
    ...
    """
    if ctx:
        await ctx.info(f"Performing semantic search for: '{query}' ({search_mode} mode)")
            ...
            try:
                ...
                if exclude_same_series:
                    ...
                    if (ref_series_id and result_series_id == ref_series_id) or \
                       (ref_title and result_title and 
                        any(word in result_title.lower() for word in ref_title.lower().split()[:2])):
                        continue
                ...
                result = {
                    ...
                    # Similarity analysis
                    "shared_genres": list(set(payload.get("genres", []) or []) & 
                                        set(ref_payload.get("genres", []) or [])),
                    "shared_studios": list(set(payload.get("studios", []) or []) & 
                                         set(ref_payload.get("studios", []) or [])),
                    "year_difference": abs((payload.get("year") or 0) - (ref_payload.get("year") or 0)),
...
async def anime_vector_stats(
    ...
    try:
        ...
        stats = {
            ...
            "vector_size": collection_info.get("config", {}).get("params", {}).get("vectors", {}),
            "distance_metric": collection_info.get("config", {}).get("params", {}).get("distance"),
            "indexing_threshold": collection_info.get("config", {}).get("params", {}).get("indexing_threshold"),
            ...
            "indexing_status": "indexed" if collection_info.get("indexed_vectors_count", 0) > 0 else "not_indexed",
            
            # Memory and performance
            "memory_usage_mb": collection_info.get("memory_usage_bytes", 0) / (1024 * 1024),
