...
async def search_anime_kitsu(
    ...
    # Status filtering
    status: Optional[Literal["current", "finished", "tba", "unreleased", "upcoming"]] = None,
    ...
    streamers: Optional[List[Literal["Crunchyroll", "Funimation", "Netflix", "Hulu", "Amazon Prime", "Disney+", "HBO Max", "Apple TV+", "Paramount+", "Peacock", "Tubi", "VRV", "AnimeLab", "Wakanim", "ADN", "Other"]]] = None,
    
    # Sorting and pagination
    sort: Optional[Literal["popularityRank", "ratingRank", "startDate", "endDate", "createdAt", "updatedAt", "-popularityRank", "-ratingRank", "-startDate", "-endDate", "-createdAt", "-updatedAt"]] = None,
    limit: int = 20,
    offset: int = 0,
    ...
    try:
        ...
        params = {
            "filter": filters,
            "page": {
                "limit": min(limit, 20),  # Kitsu max limit
                "offset": offset
            }
        }
        ...
        for raw_result in raw_results:
            try:
                ...
                streaming_links = relationships.get("streamingLinks", {}).get("data", [])
                if streaming_links:
                    result["streaming_platforms"] = [
                        link.get("attributes", {}).get("subs", []) for link in streaming_links
                    ]
...
async def get_anime_kitsu(
    ...
    try:
        ...
        # Execute request
        raw_result = await kitsu_client.get_anime_by_id(kitsu_id, include=include_params)
        ...
        if ctx:
            await ctx.info(f"Retrieved comprehensive Kitsu data for '{result['title']}'")
