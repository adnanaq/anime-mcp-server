...
class DetailedAnimeDetailsInput(BaseModel):
    ...
    cross_platform_enrich: bool = Field(default=True, description="Enrich with cross-platform data")
...
class DetailedSimilarInput(BaseModel):
    ...
    cross_platform_enrich: bool = Field(default=True, description="Enrich results with cross-platform data")
...
def register_detailed_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    async def search_anime_detailed(input: DetailedSearchInput) -> DetailedSearchResponse:
        """
        ...
        try:
            ...
            for i, result in enumerate(search_results):
                ...
                platform_results = []
                if platform == "jikan":
                    platform_results = [_transform_jikan_to_detailed(item) for item in result.get("data", [])]
                elif platform == "mal":
                    platform_results = [_transform_mal_to_detailed(item) for item in result.get("data", [])]
                elif platform == "anilist":
                    platform_results = [_transform_anilist_to_detailed(item) for item in result.get("data", [])]
            ...
            if input.order_by == "score":
                ...
                deduplicated_results.sort(key=lambda x: x.aired_from or "", reverse=True)
    ...
    @mcp.tool()
    async def get_anime_detailed(input: DetailedAnimeDetailsInput) -> DetailedAnimeResult:
        """
        ...
        try:
            ...
            if input.include_staff:
                detailed_result = await _add_staff_information(detailed_result, input.platform)
            
            if input.include_characters:
                detailed_result = await _add_character_information(detailed_result, input.platform)
            
            if input.include_relations:
                detailed_result = await _add_related_anime(detailed_result, input.platform)
    ...
    @mcp.tool()
    async def find_similar_anime_detailed(input: DetailedSimilarInput) -> DetailedSearchResponse:
        """
        ...
        try:
            ...
            reference_anime = await get_anime_detailed(DetailedAnimeDetailsInput(
                id=input.anime_id,
            ...
            if input.cross_platform_enrich:
                ...
                for result in similar_results:
                    enriched_result = await _enrich_with_cross_platform_data(result, "jikan")
            ...
            return DetailedSearchResponse(
                ...
                sources_used=["jikan", "semantic"] if input.use_semantic_search else ["jikan"]
            )
    ...
    async def advanced_anime_analysis(
        ...
        try:
            ...
            detailed_anime = await get_anime_detailed(DetailedAnimeDetailsInput(
                id=anime_id,
                platform=platform,
                include_staff=True,
            ...
            analysis = {
                ...
                "cross_platform_comparison": await _cross_platform_comparison(detailed_anime),
                "genre_analysis": _analyze_genres(detailed_anime),
                "temporal_analysis": _analyze_temporal_context(detailed_anime),
                "recommendation_factors": _extract_recommendation_factors(detailed_anime),
                "processing_time_ms": int((datetime.now() - start_time).total_seconds() * 1000)
            }
...
def _build_mal_search_params(input: DetailedSearchInput) -> Dict[str, Any]:
    ...
    fields = ["id", "title", "main_picture", "alternative_titles", "start_date", 
             "end_date", "synopsis", "mean", "rank", "popularity", "num_episodes",
             "status", "genres", "media_type", "studios", "rating"]
...
def _deduplicate_anime_results(results: List[DetailedAnimeResult]) -> List[DetailedAnimeResult]:
