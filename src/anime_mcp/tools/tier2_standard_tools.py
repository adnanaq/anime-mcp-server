...
def register_standard_tools(mcp: FastMCP) -> None:
    ...
    async def search_anime_standard(
        ...
        try:
            ...
            if input.min_score or input.max_score:
                filters_applied["score_range"] = (
                    f"{input.min_score or 0}-{input.max_score or 10}"
                )
            if input.genres:
    ...
    @mcp.tool()
    async def get_anime_standard(
        input: StandardAnimeDetailsInput,
    ) -> StandardAnimeResult:
        """
    ...
    async def find_similar_anime_standard(
        ...
        try:
            ...
            reference_anime = await get_anime_standard(
                StandardAnimeDetailsInput(id=input.anime_id, platform=input.platform)
            )
            ...
            for raw_result in raw_results.get("data", []):
                ...
                standard_result.synopsis = (
                    f"[Similarity: {similarity_score:.2f}] {standard_result.synopsis}"
                )
    ...
    async def search_by_genre_standard(
        ...
        genre_mode: str = Field(
            default="any", description="Match mode: 'any' or 'all'"
        ),
        limit: int = Field(default=20, ge=1, le=50, description="Number of results"),
        min_score: Optional[float] = Field(
            None, ge=0, le=10, description="Minimum score filter"
...
def _calculate_similarity_score(
    reference: StandardAnimeResult, candidate: StandardAnimeResult, factors: List[str]
) -> float:
