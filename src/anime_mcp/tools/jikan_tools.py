...
async def get_anime_jikan(
    ...
    try:
        ...
        primary_title = (
            raw_result.get("title") or raw_result.get("title_english") or "Unknown"
        )
        ...
        if ctx:
            await ctx.info(
                f"Retrieved comprehensive Jikan data for '{result['title']}'"
            )
        ...
        for raw_result in raw_results:
            try:
                ...
                primary_title = (
                    ...
                    or raw_result.get("title_english")
                    or "Unknown"
                )
                ...
                result = {
                    ...
                    "status": raw_result.get("status"),
                    "genres": [
                        genre.get("name") for genre in raw_result.get("genres", [])
                    ],
                    "studios": [
                        studio.get("name") for studio in raw_result.get("studios", [])
                    ],
                    "synopsis": raw_result.get("synopsis"),
                    "image_url": raw_result.get("images", {})
                    .get("jpg", {})
                    .get("image_url"),
                    # Jikan seasonal data
