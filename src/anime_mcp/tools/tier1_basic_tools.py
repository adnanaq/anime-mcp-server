...
class BasicAnimeDetailsInput(BaseModel):
    ...
    platform: str = Field(
        default="jikan",
        description="Platform to fetch from (jikan, mal, anilist, kitsu)",
    )
...
def register_basic_tools(mcp: FastMCP) -> None:
    ...
    async def find_similar_anime_basic(input: BasicSimilarInput) -> BasicSearchResponse:
        ...
        try:
            ...
            reference_anime = await get_anime_basic(
                BasicAnimeDetailsInput(id=input.anime_id, platform=input.platform)
            )
            ...
            similar_params = {
                "limit": input.limit,
                "type": (
                    reference_anime.type.value.lower() if reference_anime.type else None
                ),
                "genres": (
                    reference_anime.genres[:3] if reference_anime.genres else None
                ),  # Top 3 genres
            }

            # Search for similar anime
            if input.platform == "jikan":
                raw_results = await jikan_client.search_anime(
                    **{k: v for k, v in similar_params.items() if v}
                )
            else:
