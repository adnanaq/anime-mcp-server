...
class JikanService(BaseExternalService):
    ...
    async def get_anime_details(
        self, anime_id: int, correlation_id: Optional[str] = None
        ...
        try:
            logger.info(
                "Jikan anime details: anime_id=%d, correlation_id=%s",
                anime_id,
                correlation_id,
            )
            ...
            logger.error(
                "Jikan anime details failed: %s, correlation_id=%s", e, correlation_id
            )
            raise

    async def get_seasonal_anime(
        self, year: int, season: str, correlation_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            logger.info(
                "Jikan seasonal: year=%d, season=%s, correlation_id=%s",
                year,
                season,
                correlation_id,
            )
            ...
            logger.error(
                "Jikan seasonal failed: %s, correlation_id=%s", e, correlation_id
            )
            raise
    ...
    async def get_current_season(
        ...
        try:
            logger.info(
                "Jikan current season: year=%d, season=%s, correlation_id=%s",
                year,
                season,
                correlation_id,
            )
            ...
            logger.error(
                "Jikan current season failed: %s, correlation_id=%s", e, correlation_id
            )
            raise
    ...
    async def get_anime_statistics(
        ...
        try:
            logger.info(
                "Jikan anime statistics: anime_id=%d, correlation_id=%s",
                anime_id,
                correlation_id,
            )
            return await self.client.get_anime_statistics(
                anime_id, correlation_id=correlation_id
            )
        except Exception as e:
            logger.error(
                "Jikan anime statistics failed: %s, correlation_id=%s",
                e,
                correlation_id,
            )
            raise
    ...
    async def get_top_anime(
        ...
        try:
            logger.info(
                "Jikan top anime: type=%s, filter=%s, correlation_id=%s",
                anime_type,
                filter_type,
                correlation_id,
            )
            ...
            logger.error(
                "Jikan top anime failed: %s, correlation_id=%s", e, correlation_id
            )
            raise
    ...
    async def get_random_anime(
        ...
        try:
            ...
            logger.error(
                "Jikan random anime failed: %s, correlation_id=%s", e, correlation_id
            )
            raise
    ...
    async def get_anime_recommendations(
        ...
        try:
            logger.info(
                "Jikan recommendations: anime_id=%d, correlation_id=%s",
                anime_id,
                correlation_id,
            )
            ...
            logger.error(
                "Jikan recommendations failed: %s, correlation_id=%s", e, correlation_id
            )
            raise
    ...
    async def get_anime_characters(
        ...
        try:
            logger.info(
                "Jikan characters: anime_id=%d, correlation_id=%s",
                anime_id,
                correlation_id,
            )
            ...
            logger.error(
                "Jikan characters failed: %s, correlation_id=%s", e, correlation_id
            )
            raise
    ...
    async def get_anime_staff(
        ...
        try:
            logger.info(
                "Jikan staff: anime_id=%d, correlation_id=%s", anime_id, correlation_id
            )
    ...
    async def get_schedules(
        ...
        try:
            logger.info(
                "Jikan schedules: day=%s, correlation_id=%s", filter_day, correlation_id
            )
            ...
            logger.error(
                "Jikan schedules failed: %s, correlation_id=%s", e, correlation_id
            )
            raise

    async def get_genres(
        self, filter_name: Optional[str] = None, correlation_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            logger.info(
                "Jikan genres: filter=%s, correlation_id=%s",
                filter_name,
                correlation_id,
            )
            ...
            logger.error(
                "Jikan genres failed: %s, correlation_id=%s", e, correlation_id
            )
            raise
    ...
    def _validate_search_parameters(
        ...
        if sort and sort not in valid_sort:
            raise ValueError(f"Invalid sort '{sort}'. Must be one of: {valid_sort}")
