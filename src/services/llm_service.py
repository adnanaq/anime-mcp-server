...
class SearchIntent(BaseModel):
    ...
    query: str = Field(..., description="Cleaned search query without limit/filter words")
    ...
    year_range: Optional[List[int]] = Field(None, description="Year range [start_year, end_year]")
    anime_types: List[str] = Field(default_factory=list, description="Types: TV, Movie, OVA, Special, etc.")
    studios: List[str] = Field(default_factory=list, description="Specific animation studios")
    exclusions: List[str] = Field(default_factory=list, description="Things to exclude/avoid")
    mood_keywords: List[str] = Field(default_factory=list, description="Mood descriptors: dark, light, serious, funny")
...
class LLMService:
        ...
        try:
            if self.provider == LLMProvider.OPENAI:
                ...
                if api_key:
                    ...
                    logger.warning("Anthropic API key not found - LLM features disabled")
    ...
    async def extract_search_intent(self, user_query: str) -> SearchIntent:
        ...
        try:
            ...
            logger.error(f"LLM extraction failed: {e}, falling back to basic extraction")
            return self._fallback_extraction(user_query)
        ...
        try:
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Extract search parameters from: {user_query}"}
                ],
                response_format={
                    "type": "json_schema",
                    "json_schema": {
            ...
            for field in ['genres', 'anime_types', 'studios', 'exclusions', 'mood_keywords']:
...
Respond with valid JSON only."""
        ...
        try:
            response = await self.client.messages.create(
                ...
                messages=[
                    {"role": "user", "content": f"Extract search parameters: {user_query}"}
                ],
                max_tokens=500,
                temperature=0.1
            )
