...
class ConversationalAgent:
    ...
    def _build_workflow_graph(self) -> WorkflowGraph:
        ...
        nodes = [
            ...
            WorkflowNode("synthesis", self._synthesis_node, "Synthesize recommendations"),
    ...
    async def process_user_message(self, state: ConversationState, message: str) -> ConversationState:
    ...
    async def _execute_workflow(self, state: ConversationState, start_node: str) -> ConversationState:
    ...
    async def _understand_node(self, state: ConversationState) -> ConversationState:
        ...
        if state.current_context:
            ...
            new_context.image_data = state.current_context.image_data or new_context.image_data
    ...
    async def _search_node(self, state: ConversationState) -> ConversationState:
        ...
        try:
            ...
            step = WorkflowStep(
                ...
                tool_name="search_anime" if not context.image_data else "search_multimodal_anime",
    ...
    async def _reasoning_node(self, state: ConversationState) -> ConversationState:
        ...
        if not state.current_context or not state.current_context.results:
            ...
            if years:
                ...
                if valid_years:
                    year_range = f"{min(valid_years)}-{max(valid_years)}" if len(set(valid_years)) > 1 else str(valid_years[0])
                    reasoning_parts.append(f"Year range: {year_range}")
            
            reasoning = "Analysis: " + "; ".join(reasoning_parts) if reasoning_parts else "Diverse results found"
        ...
        for result in results:
            ...
            if preferences:
                if preferences.favorite_genres:
                    ...
                    genre_match = len(set(preferences.favorite_genres) & set(result_genres))
                    score += genre_match * 0.1
                
                if preferences.favorite_studios:
                    result_studios = result.get("studios", [])
                    studio_match = len(set(preferences.favorite_studios) & set(result_studios))
    ...
    async def _response_node(self, state: ConversationState) -> ConversationState:
        ...
        if not state.current_context or not state.current_context.results:
            ...
            response_parts = [f"I found {len(state.current_context.results)} anime for you. Here are the top recommendations:"]
        ...
        assistant_msg = WorkflowMessage(
            ...
            tool_results=state.current_context.results[:3] if state.current_context else []
        )
    ...
    def _extract_preferences(self, message: str, existing_prefs: Optional[UserPreferences]) -> Optional[UserPreferences]:
        ...
        if detected_genres or detected_studios:
            if existing_prefs:
                ...
                all_studios = list(set(existing_prefs.favorite_studios + detected_studios))
                ...
                return UserPreferences(
                    favorite_genres=detected_genres,
                    favorite_studios=detected_studios
                )
        
        return None
    
    async def _handle_workflow_error(self, state: ConversationState, node_name: str, error: Exception) -> None:
...
class AnimeWorkflowEngine:
    ...
    async def get_conversation_summary(self, state: ConversationState) -> str:
        ...
        user_messages = [msg.content for msg in state.messages if msg.message_type == MessageType.USER]
        assistant_messages = [msg.content for msg in state.messages if msg.message_type == MessageType.ASSISTANT]
        ...
        if state.user_preferences and state.user_preferences.favorite_genres:
            summary_parts.append(f"Preferred genres: {', '.join(state.user_preferences.favorite_genres[:3])}")
