...
class SendAPIParallelRouter:
    ...
    async def _query_analyzer_node(self, state: SendAPIState) -> SendAPIState:
        ...
        logger.info(
            f"Query complexity: {complexity_score:.2f}, strategy: {route_strategy.value}"
        )
    ...
    def _send_api_router(self, state: SendAPIState) -> List[Send]:
        ...
        if parallel_routes:
            ...
            logger.warning(
                "No parallel routes generated, falling back to result merger"
            )
    ...
    async def _execute_agent_search(
        self, state: SendAPIState, agent_name: str, tool_name: str
    ) -> SendAPIState:
        ...
        try:
            ...
            if tool_name in self.mcp_tools:
                ...
                result = await asyncio.wait_for(
                    (
                        tool_func.ainvoke(search_params)
                        if hasattr(tool_func, "ainvoke")
                        else tool_func(**search_params)
                    ),
        ...
        for agent_name, agent_result in route_results.items():
            if agent_result.get("success", False):
                ...
                total_processing_time = max(
                    total_processing_time, agent_result.get("processing_time_ms", 0)
                )
    ...
    async def _analyze_query_complexity(
        self, query: str, image_data: Optional[str] = None
    ) -> float:
        ...
        criteria_count = sum(
            1 for keyword in criteria_keywords if keyword.lower() in query.lower()
        )
    ...
    async def process_conversation(
        ...
        """
        logger.info(
            f"Processing conversation with Send API parallel routing for session {session_id}"
        )
            ...
            "workflow_steps": [
                {
                    ...
                    "route_strategy": (
                        result.get("route_strategy", {}).value
                    ...
                    "successful_agents": len(
                        merged_results.get("successful_agents", [])
                    ),
