...
from .send_api_router import QueryComplexity, SendAPIParallelRouter
from .swarm_agents import MultiAgentSwarm
...
class SuperStepPhase(Enum):
    ...
    COMPREHENSIVE_BATCH = (
        "comprehensive_batch"  # Super-step 2: All agents + enhancement (300-1000ms)
    )
...
class SuperStepParallelExecutor:
    ...
    def _initialize_super_step_configs(self) -> Dict[SuperStepPhase, SuperStepConfig]:
        ...
        return {
            ...
            SuperStepPhase.ENHANCEMENT_BATCH: SuperStepConfig(
                ...
                max_retries=2,
                agent_batch=[
                    "RatingCorrelation_Agent",
                    "StreamingAvailability_Agent",
                    "ReviewAggregation_Agent",
                ],
                parallel_limit=3,
                rollback_enabled=True,
                checkpoint_enabled=True,
    ...
    def _build_execution_graph(self) -> CompiledStateGraph:
        ...
        workflow.add_node(
            "execute_comprehensive_batch", self._execute_comprehensive_batch
        )
        ...
        workflow.add_conditional_edges(
            ...
            {"finalize": "finalize_results", "rollback": "handle_rollback"},
        )
    ...
    async def _analyze_complexity(self, state: SuperStepState) -> SuperStepState:
        ...
        try:
            ...
            complexity = await self.send_api_router._analyze_query_complexity(
                state["query"]
            )
    ...
    async def _execute_comprehensive_batch(
        self, state: SuperStepState
    ) -> SuperStepState:
    ...
    async def _execute_super_step(
        self, state: SuperStepState, phase: SuperStepPhase
    ) -> SuperStepState:
        ...
        logger.info(
            f"Executing super-step: {phase.value} with {len(config.agent_batch)} agents"
        )
        ...
        try:
            ...
            error_msg = (
                f"Super-step {phase.value} timed out after {config.timeout_ms}ms"
            )
            errors.append(error_msg)
            logger.error(error_msg)
        except Exception as e:
        ...
        super_step_result = SuperStepResult(
            phase=phase,
            status=(
                SuperStepStatus.COMPLETED
                if success_count > 0
                else SuperStepStatus.FAILED
            ),
            results=results,
        ...
        if (
            ...
            and success_count == 0
            and len(config.agent_batch) > 1
        ):
            state["rollback_triggered"] = True
            logger.warning(
                f"Super-step {phase.value} failed completely, triggering rollback"
            )
    ...
    async def _finalize_results(self, state: SuperStepState) -> SuperStepState:
        ...
        try:
            ...
            merged_result = await self._execute_agent_with_timeout(
                "ResultMerger_Agent", json.dumps(state["cumulative_results"]), 500
            )
            ...
            if merged_result.get("status") == "error":
                ...
                logger.info(
                    f"Results finalized with {len(state['cumulative_results'])} total results"
                )
            ...
            return {
                ...
                "performance_metrics": {
                    ...
                    "total_agents_executed": sum(
                        r.agent_count for r in final_state["super_step_results"]
                    ),
                    "successful_agents": sum(
                        r.success_count for r in final_state["super_step_results"]
                    ),
