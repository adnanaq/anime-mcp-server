...
from typing import (
    ...
    AsyncGenerator, NamedTuple, Protocol
)
...
from .workflow_state import AnimeSwarmState, QueryIntent, WorkflowResult
...
class ConversationContextMemory:
    ...
    def __init__(self, max_sessions: int = 1000, session_ttl_hours: int = 24):
        ...
        logger.info(f"ConversationContextMemory initialized: max_sessions={max_sessions}, ttl={session_ttl_hours}h")
    ...
    def update_user_profile(self, user_id: str, interaction_data: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Updated user profile for {user_id}, interactions: {profile.interaction_count}")
...
class AgentHandoffOptimizer:
    ...
    def learn_from_execution(
        ...
        if execution_result.get('execution_time_ms'):
            pattern.average_completion_time = (
                (pattern.average_completion_time * (pattern.usage_count - 1) + 
                 execution_result['execution_time_ms']) / pattern.usage_count
            )
        ...
        pattern.success_rate = (
            (pattern.success_rate * (pattern.usage_count - 1) + 
             (1.0 if result_success else 0.0)) / pattern.usage_count
        )
        
        # Learn context triggers
        if context.get('intent_type') and context['intent_type'] not in pattern.optimal_for_intents:
        ...
        for agent in agent_sequence:
            self.agent_success_rates[agent] = (
                (self.agent_success_rates[agent] * 0.9) + (0.1 if result_success else 0.0)
            )
        ...
        for i in range(len(agent_sequence) - 1):
            ...
            self.agent_handoff_success[handoff_key] = (
                (self.agent_handoff_success[handoff_key] * 0.9) + (0.1 if result_success else 0.0)
            )
        
        logger.debug(f"Learned from sequence: {sequence_key}, success: {result_success}")
    ...
    def get_optimal_sequence(
        ...
        matching_patterns = [
            pattern for pattern in self.sequence_patterns.values()
            if query_intent in pattern.optimal_for_intents and pattern.success_rate > 0.5
        ]
    ...
    def get_best_handoff_target(self, current_agent: str, context: Dict[str, Any]) -> Optional[str]:
        """Get the best next agent for handoff from current agent."""
        # Find the best handoff target based on success rates
        possible_handoffs = [
            (target, rate) for (source, target), rate in self.agent_handoff_success.items()
...
class RoutingMemoryStore:
    ...
    def __init__(self, max_patterns: int = 10000):
        ...
        self.performance_index: List[Tuple[str, float]] = []  # (pattern_id, success_rate)
    ...
    def store_pattern(self, pattern: QueryPattern) -> None:
        ...
        logger.debug(f"Stored pattern {pattern.pattern_id}, intent: {pattern.intent_type}")
    ...
    def find_similar_patterns(
        ...
        for pattern_id in candidate_pattern_ids:
            ...
            similarity = len(common_words) / max(len(query_lower.split()), len(pattern_text.split()))
    ...
    def get_best_patterns_for_intent(self, intent_type: str, limit: int = 3) -> List[QueryPattern]:
        """Get the best performing patterns for a specific intent type."""
        candidate_pattern_ids = self.intent_patterns.get(intent_type, [])
        
        patterns = [
            self.query_patterns[pid] for pid in candidate_pattern_ids 
    ...
    def _update_performance_index(self, pattern_id: str, success_rate: float) -> None:
        ...
        self.performance_index = [(pid, sr) for pid, sr in self.performance_index if pid != pattern_id]
...
class StatefulRoutingEngine:
        ...
        try:
            ...
            if similar_patterns:
                ...
                if best_pattern.successful_agent_sequences:
                    routing_decision["agent_sequence"] = best_pattern.successful_agent_sequences[0]
                    routing_decision["confidence"] = best_pattern.success_rate
                    routing_decision["reasoning"].append(f"Using pattern {best_pattern.pattern_id} (success: {best_pattern.success_rate:.2f})")
            ...
            if not routing_decision["agent_sequence"]:
                ...
                if optimal_sequence:
                    ...
                    routing_decision["reasoning"].append("Using learned optimal agent sequence")
            ...
            if user_id:
                ...
                if user_profile and user_profile.optimal_agent_sequences:
                    ...
                    routing_decision["agent_sequence"] = user_profile.optimal_agent_sequences[0]
                    routing_decision["confidence"] = user_profile.confidence_score
                    routing_decision["reasoning"].append("Using user preference profile")
        ...
        try:
            ...
            if user_id:
                interaction_data = {
                    ...
                    "query_complexity": execution_result.get('intent_analysis', {}).get('workflow_complexity', 'moderate'),
            ...
            logger.debug(f"Learned from execution: pattern={pattern_id}, success={pattern.success_rate}")
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        avg_success_rate = sum(1 for p in recent_performance if p["success"]) / max(len(recent_performance), 1)
        avg_response_time = sum(p["response_time"] for p in recent_performance) / max(len(recent_performance), 1)
        ...
        return {
            ...
            "memory_efficiency": "Good" if avg_success_rate > 0.8 else "Needs Improvement"
        }
