...
try:
    ...
    def create_swarm(*args, **kwargs):
        raise ImportError(
            "langgraph-swarm not available. Install with: pip install langgraph-swarm"
        )
...
class EnhancementAgentType(Enum):
    ...
    STREAMING_AVAILABILITY_AGENT = (
        "streaming_availability_agent"  # Multi-platform streaming
    )
...
You are the terminal agent - synthesize all gathered data into the final response.""",
        ...
        for agent_name, definition in self.agent_definitions.items():
            ...
            for target_agent in definition.handoff_targets:
                try:
                    ...
                    logger.warning(
                        f"Could not create handoff tool for {target_agent}: {e}"
                    )
    ...
    def _create_swarm_system(self) -> Any:
        ...
        try:
            ...
            valid_agents = {
                name: agent for name, agent in self.agents.items() if agent is not None
            }
            ...
            swarm = create_swarm(
                list(valid_agents.values()), default_active_agent="query_analysis_agent"
            )
    ...
    async def process_conversation(
        ...
        """
        logger.info(
            f"Processing conversation with Multi-Agent Swarm for session {session_id}"
        )
