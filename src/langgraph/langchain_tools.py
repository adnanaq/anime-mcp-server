...
class SearchAnimeInput(BaseModel):
    """Input schema for search_anime tool."""
...
class RecommendAnimeInput(BaseModel):
    """Input schema for recommend_anime tool."""
    genres: Optional[str] = Field(None, description="Comma-separated genre list")
    year: Optional[int] = Field(None, description="Preferred release year")
    anime_type: Optional[str] = Field(None, description="Anime type (TV, Movie, etc.)")
...
def create_anime_langchain_tools(mcp_tools: Dict[str, Callable[..., Any]]) -> List[Any]:
    ...
    if "search_anime" in mcp_tools:
        ...
        async def search_anime(query: str, limit: int = 10) -> Any:
            ...
            validated_input = SearchAnimeInput(query=query, limit=limit)
    ...
    # Recommend anime tool
    if "recommend_anime" in mcp_tools:
        @tool("recommend_anime", args_schema=RecommendAnimeInput)
        async def recommend_anime(
            ...
            validated_input = RecommendAnimeInput(
                genres=genres, year=year, anime_type=anime_type, limit=limit
            )
            return await mcp_tools["recommend_anime"](**validated_input.model_dump())
        tools.append(recommend_anime)
    ...
    if "search_anime_by_image" in mcp_tools:
        ...
        async def search_anime_by_image(image_data: str, limit: int = 10) -> Any:
            ...
            return await mcp_tools["search_anime_by_image"](**validated_input.model_dump())
    ...
    if "find_visually_similar_anime" in mcp_tools:
        ...
        async def find_visually_similar_anime(anime_id: str, limit: int = 10) -> Any:
            ...
            return await mcp_tools["find_visually_similar_anime"](**validated_input.model_dump())
    ...
    if "search_multimodal_anime" in mcp_tools:
        ...
        async def search_multimodal_anime(
            ...
            return await mcp_tools["search_multimodal_anime"](**validated_input.model_dump())
...
class AnimeToolNodeWorkflow:
    ...
    This replaces the MCPAdapterRegistry pattern with native LangGraph ToolNode
    integration, providing better performance and maintainability.
    """
    ...
    async def _chatbot_node(self, state: ToolNodeConversationState) -> ToolNodeConversationState:
        ...
        if isinstance(last_message, ToolMessage):
            ...
            summary_message = AIMessage(
                content=self._create_response_from_tool_result(tool_result, last_message.tool_call_id)
            )
    ...
    def _needs_anime_search(self, message_content: str) -> bool:
        ...
        search_keywords = [
            "find", "search", "recommend", "anime", "show", "series", "movie",
    ...
    def _extract_search_parameters(self, message_content: str) -> Dict[str, Any]:
        ...
        import re
        limit_match = re.search(r'\b(\d+)\s*(?:results?|anime|shows?)', message_content.lower())
        if limit_match:
            params["limit"] = min(int(limit_match.group(1)), 50)
            
        return params

    def _create_response_from_tool_result(self, tool_result: Any, tool_call_id: str) -> str:
