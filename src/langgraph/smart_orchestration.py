...
class QueryChainOrchestrator:
    ...
    async def execute_chain(
        self, chain: QueryChain, state: SmartOrchestrationState
    ) -> Dict[str, Any]:
        ...
        limit = (
            state.current_context.limit
            if state.current_context and state.current_context.limit
            else 10
        )
        ...
        for i, query in enumerate(chain.queries):
            ...
            if strategy == "semantic":
                result = await self.adapter_registry.invoke_tool(
                    "search_anime", {"query": query, "limit": limit}
                )
    ...
    def _determine_search_strategy(
        self, query: str, chain: QueryChain, index: int
    ) -> str:
...
class ResultRefinementEngine:
    ...
    async def refine_results(
        ...
        for iteration in range(criteria.max_iterations):
            ...
            focused_results = self._apply_focus_filtering(
                filtered_results, criteria.focus_areas
            )

            # Remove excluded content
            final_results = self._apply_exclusions(
                focused_results, criteria.exclusion_criteria
            )
            ...
            if (
                avg_confidence >= criteria.min_confidence
                and len(final_results) >= criteria.target_result_count
            ):
                break
    ...
    def _apply_quality_filters(
        self, results: List[Dict[str, Any]], criteria: RefinementCriteria
    ) -> List[Dict[str, Any]]:
    ...
    def _apply_focus_filtering(
        ...
        for result in results:
            ...
            for focus in focus_areas:
                ...
                if (
                    focus_lower in tags
                    or focus_lower in studios
                    or focus_lower in title
                ):
                    matches_focus = True
                    break
    ...
    def _apply_exclusions(
        self, results: List[Dict[str, Any]], exclusions: List[str]
    ) -> List[Dict[str, Any]]:
        """Remove excluded content."""
        if not exclusions:
            return results
        ...
        for result in results:
            ...
            for exclusion in exclusions:
                ...
                if (
                    exclusion_lower in tags
                    or exclusion_lower in studios
                    or exclusion_lower in title
                ):
                    is_excluded = True
                    break
    ...
    async def _expand_search(
        ...
        if anime_id:
            similar_results = await self.adapter_registry.invoke_tool(
                "find_similar_anime", {"anime_id": anime_id, "limit": needed_count}
            )
...
class ConversationFlowManager:
    ...
    def create_discovery_flow(self, initial_query: str) -> ConversationFlow:
        ...
        return ConversationFlow(
            ...
            stages=[
                {
                    "name": "initial_search",
                    "description": "Initial search based on user query",
                },
                {
                    "name": "preference_extraction",
                    "description": "Extract user preferences from context",
                },
                {
                    "name": "result_refinement",
                    "description": "Refine results based on preferences",
                },
                {
                    "name": "similarity_exploration",
                    "description": "Explore similar content",
                },
                {
                    "name": "final_recommendation",
                    "description": "Provide final recommendations",
                },
            ],
            branch_conditions={
    ...
    async def execute_flow_stage(
        self, flow: ConversationFlow, state: SmartOrchestrationState
    ...
    async def _execute_initial_search(
        ...
        results = await self.adapter_registry.invoke_tool(
            "search_anime", {"query": state.current_context.query, "limit": 15}
        )
    ...
    async def _execute_result_refinement(
        ...
        criteria = RefinementCriteria(
            min_confidence=0.7, max_iterations=2, target_result_count=10
        )
        ...
        refined_results, refinement_steps = await refinement_engine.refine_results(
            state.current_context.results, criteria, state.current_context
        )
    ...
    async def _execute_multimodal_analysis(
        ...
        if state.current_context.query and state.current_context.image_data:
            ...
            results = await self.adapter_registry.invoke_tool(
                ...
                {"image_data": state.current_context.image_data, "limit": 10},
            )
        else:
            # Text-only search
            results = await self.adapter_registry.invoke_tool(
                "search_anime", {"query": state.current_context.query, "limit": 10}
            )
    ...
    def _extract_preferences_from_context(
        self, state: SmartOrchestrationState
    ) -> Dict[str, Any]:
        """Extract user preferences from conversation context."""
        preferences = {
            "favorite_genres": [],
        ...
        if state.current_context and state.current_context.results:
            ...
            high_scored = [
                r for r in state.current_context.results if r.get("score", 0) > 0.8
            ]
            genre_counts = {}
        ...
        if branch_key in flow.branch_conditions:
            ...
            if "min_results" in conditions:
                if (
                    ...
                    or len(state.current_context.results) < conditions["min_results"]
                ):
                    return None
            ...
            if "confidence_threshold" in conditions:
                if not state.current_context or not state.current_context.results:
                    return None
                avg_confidence = sum(
                    r.get("score", 0.0) for r in state.current_context.results
                ) / len(state.current_context.results)
...
class SmartOrchestrationEngine:
    ...
    async def process_complex_conversation(
        self, state: SmartOrchestrationState, message: str
    ) -> SmartOrchestrationState:
        """Process a complex conversation with smart orchestration."""
        step_start = time.time()

        # Add user message
        user_message = WorkflowMessage(message_type=MessageType.USER, content=message)
        ...
        if complexity_score > 0.7:  # High complexity - use orchestration
            orchestrated_state = await self._execute_orchestrated_workflow(
                state, message
            )
    ...
    async def process_multimodal_orchestration(
        ...
        while flow.current_stage:
            ...
            if (
                not flow.current_stage
                or flow.current_stage == stage_steps[-1].tool_name
                if stage_steps
                else False
            ):
                break
        ...
        assistant_message = WorkflowMessage(
            message_type=MessageType.ASSISTANT, content=response_content
        )
    ...
    def _assess_query_complexity(
        self, message: str, state: SmartOrchestrationState
    ) -> float:
    ...
    async def _execute_orchestrated_workflow(
        ...
        if all_results:
            criteria = RefinementCriteria(
                min_confidence=0.75, max_iterations=3, target_result_count=12
            )

            refined_results, refinement_steps = (
                await self.refinement_engine.refine_results(
    ...
    async def _execute_standard_workflow(
        ...
        # Perform search
        results = await self.adapter_registry.invoke_tool(
            "search_anime", {"query": message, "limit": limit}
        )
    ...
    def _generate_orchestrated_response(
        self, state: SmartOrchestrationState, chain: QueryChain
    ) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            response_parts.append(f"{i}. **{title}** (Match: {score:.1%})")
            if tags:
    ...
    def _generate_multimodal_response(self, state: SmartOrchestrationState) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            response_parts.append(f"{i}. **{title}** (Similarity: {score:.1%})")
    ...
    def _generate_standard_response(self, state: SmartOrchestrationState) -> str:
        ...
        response_parts = [f"I found {len(results)} anime for your search:", ""]

        for i, result in enumerate(results, 1):
            title = result.get("title", "Unknown")
            score = result.get("score", 0.0)

            response_parts.append(f"{i}. **{title}** (Relevance: {score:.1%})")
