...
class BasicAnimeResult(BaseModel):
    ...
    type: Optional[AnimeType] = Field(
        None, description="Media type (TV, Movie, OVA, etc.)"
    )
    ...
    synopsis: Optional[str] = Field(
        None, description="Short plot summary", max_length=300
    )
...
class StandardAnimeResult(BaseModel):
    ...
    duration: Optional[int] = Field(
        None, ge=0, description="Episode duration (minutes)"
    )
...
class PlatformRating(BaseModel):
    ...
    score: Optional[float] = Field(
        None, ge=0, le=10, description="Normalized score (0-10)"
    )
...
class ComprehensiveAnimeResult(BaseModel):
    ...
    platform_ids: Dict[str, Union[int, str]] = Field(
        default={}, description="Platform-specific IDs"
    )
    ...
    platform_ratings: List[PlatformRating] = Field(
        default=[], description="Multi-platform ratings"
    )
    average_score: Optional[float] = Field(
        None, ge=0, le=10, description="Average across platforms"
    )
    popularity: Optional[int] = Field(None, ge=0, description="Popularity ranking")
...
class ComprehensiveSearchResponse(BaseModel):
    ...
    confidence_score: Optional[float] = Field(
        None, ge=0, le=1, description="Result confidence"
    )
...
class SearchStats(BaseModel):
    ...
    sources_health: Dict[str, bool] = Field(
        default={}, description="Source availability"
    )
    last_updated: datetime = Field(
        default_factory=datetime.now, description="Stats timestamp"
    )
