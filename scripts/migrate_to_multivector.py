...
async def run_migration(qdrant_client: QdrantClient, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        if result["migration_successful"]:
            ...
            logger.info(
                f"   💾 Backup collection: {result.get('backup_collection', 'N/A')}"
            )
            ...
            if final_count == result["preserved_vectors"]:
                logger.info(
                    f"✅ Migration verification passed: {final_count:,} documents preserved"
                )
                return True
            else:
                logger.error(
                    f"❌ Migration verification failed: expected {result['preserved_vectors']}, got {final_count}"
                )
                return False
        ...
        if isinstance(vectors_config, dict):
            if "text" in vectors_config and "image" in vectors_config:
                ...
                if results:
                    ...
                    logger.warning(
                        "⚠️  Text search returned no results (collection may be empty)"
                    )
                    return True
            else:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate anime collection to multi-vector"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
