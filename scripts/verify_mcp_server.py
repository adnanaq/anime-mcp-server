...
async def test_basic_mcp_functionality(session: ClientSession) -> bool:
    ...
    try:
        ...
        expected_tools = [
            ...
            "get_anime_stats", "recommend_anime", "search_anime_by_image",
            "find_visually_similar_anime", "search_multimodal_anime"
        ]
        ...
        if stats_result.content and stats_result.content[0].text:
            ...
            try:
                ...
                if "total_documents" in stats_data:
                    logger.info(f"   Database contains {stats_data['total_documents']:,} anime entries")
                else:
...
async def test_image_search_functionality(session: ClientSession, detailed: bool = False) -> bool:
                ...
                if test_anime_id:
                    ...
                    if similar_result.content and similar_result.content[0].text:
                        try:
                            ...
                            if isinstance(results, list):
                                logger.info(f"Visual similarity search returned {len(results)} results")
                                if detailed and results:
                                    for i, result in enumerate(results[:2], 1):
                                        title = result.get('title', 'Unknown')
                                        score = result.get('score', 0)
                                ...
                                logger.warning("Visual similarity returned non-list result")
                        except json.JSONDecodeError:
                            logger.warning("Visual similarity result is not valid JSON")
                    else:
                        logger.warning("Visual similarity search returned no content")
                else:
                    logger.warning("No anime found with image embeddings for similarity test")
            else:
                logger.warning("Cannot connect to Qdrant directly for similarity test setup")
...
async def test_database_health_and_statistics(detailed: bool = False) -> bool:
    ...
    try:
        ...
        if detailed and total_docs > 0:
            ...
            processing_rate = (processed_count / sample_size) * 100 if sample_size > 0 else 0
            ...
            if processing_rate < 50:
                logger.warning("Low image processing coverage - consider running image embedding pipeline")
...
async def run_comprehensive_tests(detailed: bool = False, skip_image_tests: bool = False):
...
if __name__ == "__main__":
    ...
    success = asyncio.run(run_comprehensive_tests(
        ...
        skip_image_tests=args.skip_image_tests
    ))
