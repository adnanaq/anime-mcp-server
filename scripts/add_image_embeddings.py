...
class ImageEmbeddingProcessor:
    ...
    async def get_anime_points_needing_images(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        logger.info(f"📊 Found {len(all_points)} anime entries needing image processing")
        logger.info(f"📊 Skipped {self.stats['skipped_no_url']} entries without picture URLs")
    ...
    async def process_image_batch(self, anime_batch: List[Dict[str, Any]]) -> List[PointStruct]:
        ...
        for anime in anime_batch:
            ...
            logger.debug(f"Processing {anime['payload'].get('title', 'Unknown')} - {picture_url}")
            ...
            if not image_embedding or all(v == 0.0 for v in image_embedding):
                ...
                logger.warning(f"Failed to generate embedding for {anime['payload'].get('title', 'Unknown')}")
                continue
    ...
    async def process_all_images(self, dry_run: bool = False, limit: Optional[int] = None) -> Dict[str, Any]:
        ...
        for i in range(0, len(anime_points), self.batch_size):
            ...
            logger.info(f"📈 Progress: {progress:.1f}% ({self.stats['processed']}/{len(anime_points)})")
        ...
        if self.stats["processed"] > 0:
            success_rate = (self.stats["successful_embeddings"] / self.stats["processed"]) * 100
...
async def verify_image_processing(qdrant_client: QdrantClient) -> Dict[str, Any]:
    ...
    try:
        ...
        if results:
            ...
            return {"success": True, "image_search_working": True, "sample_non_zero": non_zero_count}
        else:
            logger.warning("⚠️  Image search returned no results")
            return {"success": True, "image_search_working": False, "sample_non_zero": non_zero_count}
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Process anime poster images and add embeddings")
    ...
    if not getattr(qdrant_client, '_supports_multi_vector', False):
        logger.error("❌ Multi-vector support not enabled. Set ENABLE_MULTI_VECTOR=true")
        return False
    ...
    # Process images
    async with ImageEmbeddingProcessor(qdrant_client, batch_size=args.batch_size) as processor:
        result = await processor.process_all_images(dry_run=args.dry_run, limit=args.limit)
