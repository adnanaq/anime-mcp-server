...
class TestRunner:
    ...
    def validate_test_structure(self, test_files: Dict[str, List[Path]]) -> bool:
        ...
        required_tests = {
            ...
            "compatibility": ["test_refactoring_compatibility.py"],
        }
        ...
        for category, files in test_files.items():
            for test_file in files:
                try:
                    ...
                    print(
                        f"✅ Syntax valid: {test_file.relative_to(self.project_root)}"
                    )

                except SyntaxError as e:
                    print(
                        f"❌ Syntax error in {test_file.relative_to(self.project_root)}: {e}"
                    )
                    syntax_valid = False
                except Exception as e:
                    print(
                        f"⚠️ Warning: Could not validate {test_file.relative_to(self.project_root)}: {e}"
                    )
    ...
    def _validate_exception_tests(self) -> bool:
        ...
        exception_test_file = (
            self.project_root / "tests" / "unit" / "test_exceptions.py"
        )
    ...
    def _validate_mcp_tests(self) -> bool:
        ...
        mcp_test_file = (
            self.project_root / "tests" / "unit" / "mcp" / "test_mcp_server.py"
        )
    ...
    def analyze_test_coverage(self) -> Dict[str, Any]:
        ...
        for src_file in source_files:
            ...
            for test_file in all_test_files:
                if module_name in test_file.name or test_file.name.startswith(
                    f"test_{module_name}"
                ):
                    has_test = True
                    break
            ...
            if has_test:
                coverage_analysis["covered_modules"].append(
                    src_file.relative_to(self.project_root)
                )
            else:
                coverage_analysis["uncovered_modules"].append(
                    src_file.relative_to(self.project_root)
                )

        # Calculate coverage ratio
        total_modules = len(coverage_analysis["covered_modules"]) + len(
            coverage_analysis["uncovered_modules"]
        )
        if total_modules > 0:
            coverage_analysis["coverage_ratio"] = (
                len(coverage_analysis["covered_modules"]) / total_modules
            )
    ...
    def generate_test_report(self) -> str:
        ...
        report.append(
            f"## Test Structure: {'✅ PASSED' if structure_valid else '❌ FAILED'}"
        )
        report.append("")
        ...
        report.append(
            f"## Logic Validation: {'✅ PASSED' if logic_valid else '❌ FAILED'}"
        )
        report.append("")
        ...
        report.append(
            f"## Import Structure: {'✅ PASSED' if import_valid else '❌ FAILED'}"
        )
        report.append("")
        ...
        if all_passed and coverage_good:
            report.append(
                "🎉 **ALL VALIDATIONS PASSED** - Refactored code is well-tested and compatible"
            )
        elif all_passed:
            report.append(
                "✅ **VALIDATIONS PASSED** - Minor coverage improvements recommended"
            )
        else:
            report.append(
                "❌ **SOME VALIDATIONS FAILED** - Issues need to be addressed"
            )
